...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function openModal(b){
    ...
    mGal.style.display = 'block';
    mGal.innerHTML = '<div class="coming-soon-box" style="padding: 24px; box-shadow: none; border: 1px dashed var(--border-color); text-align: center; border-radius: 12px; max-width: 100%; margin-top: 16px;">'
  ...
  setTimeout(function() {
    ...
    // Animate barber cards with stagger
  ...
  var reviews = [
    ...
    {name:'Marcus L.', time:'3 weeks ago', text:'"The vibe in this shop is unmatched. Good music, great conversation, and the cuts speak for themselves. Five stars is not enough."'},
    {name:'Brandon K.', time:'1 month ago', text:'"Brought my son here for his first real haircut and they treated him like a VIP. Patient, skilled, and the lineup was cleaner than mine."'},
    {name:'Anthony M.', time:'2 months ago', text:'"Been to barbers all over Philly and Bucks County. No Push Backs is easily top tier. Professional from the booking to the final mirror check."'},
  ...
  if (carousel && dotsContainer) {
    ...
    for (var s = 0; s < totalSlides; s++) {
      ...
      for (var ri = s * perSlide; ri < Math.min((s+1) * perSlide, reviews.length); ri++) {
        var rv = reviews[ri];
        var card = document.createElement('div');
        card.className = 'review-card';
        card.innerHTML = '<div class="review-user">'
          + '<div class="user-avatar" style="background-color:'+colors[ri]+';">'+rv.name.charAt(0)+'</div>'
          + '<div class="user-info"><strong>'+rv.name+'</strong><span>'+rv.time+'</span></div>'
          + gIcon + '</div>'
    ...
    function goToSlide(n) {
      ...
      for (var di = 0; di < dots.length; di++) {
        dots[di].classList.toggle('active', di === n);
      }
      var activeSlide = carousel.querySelectorAll('.carousel-slide')[n];
      var acards = activeSlide.querySelectorAll('.review-card');
      for (var ci = 0; ci < acards.length; ci++) {
        acards[ci].style.animation = 'none';
        acards[ci].offsetHeight;
        acards[ci].style.animation = '';
      }
    }
